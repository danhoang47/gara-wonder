import { useState } from "react";

import { Chip } from "@nextui-org/react";

import { GarageActionButton } from "..";

function GarageHeader() {
    const [hasCoupon,setHasCoupon] = useState<boolean>(true);
    
    return <div className="flex justify-between items-center ">
        <div>
            <span className="text-xs font-medium">522 visited</span>
            <div className="flex gap-2 items-center">
                <p className="font-semibold text-2xl">Detail and Repairing Garage Da Nang</p>
                { hasCoupon && 
                <Chip color="primary" radius="sm" size="sm">
                    Coupon
                </Chip>
                }
            </div>
            <span className="text-xs font-medium">Khu do thi FPT, Hoa Hai, Ngu Hanh Son, Da Nang </span>
        </div>
      <GarageActionButton />
    </div>
}

export default GarageHeader;